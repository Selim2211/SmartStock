...
export default function HizliSatisPage() {
  ...
  return (
    <div className="flex h-[calc(100vh-4rem)] min-h-[520px] w-full flex-col gap-0 overflow-hidden rounded-2xl border border-zinc-200 bg-white shadow-xl dark:border-zinc-800 dark:bg-zinc-950">
      <div className="grid flex-1 grid-cols-1 overflow-hidden lg:grid-cols-[minmax(0,7fr)_minmax(0,3fr)]">
          ...
          <div className="flex shrink-0 gap-2 overflow-x-auto border-b border-zinc-200 px-4 py-3 dark:border-zinc-800">
            {categories.map((cat) => (
              <button
                ...
                className={`shrink-0 rounded-xl px-4 py-2 text-sm font-medium transition focus:outline-none focus:ring-2 focus:ring-offset-2 dark:focus:ring-offset-zinc-950 ${
                  selectedCategory === cat
                    ? "bg-[#C17F59] text-white shadow-md focus:ring-[#C17F59] dark:bg-[#C17F59] dark:text-white"
              ...
              <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-3 lg:grid-cols-3 xl:grid-cols-4">
                {filteredProducts.map((p) => (
                  <button
                    ...
                    type="button"
                    onClick={() => addToCart(p)}
                    className="group flex flex-col overflow-hidden rounded-xl border border-zinc-200 bg-zinc-50/80 text-left shadow-sm transition hover:border-[#C17F59]/60 hover:shadow-md focus:outline-none focus:ring-2 focus:ring-[#C17F59] focus:ring-offset-2 dark:border-zinc-800 dark:bg-zinc-900/80 dark:hover:border-[#C17F59]/60"
                  >
                    ...
                    <div className="flex flex-1 flex-col justify-between gap-1 p-2.5">
                      ...
                      <span className="text-sm font-semibold text-[#C17F59] dark:text-amber-400">
                        {p.price.toFixed(2)} ₺
                      </span>
                    </div>
                  </button>
        ...
        <div className="flex flex-col border-t border-zinc-200 bg-zinc-50/50 dark:border-zinc-800 dark:bg-zinc-900/50 lg:border-t-0 lg:border-l">
          <div className="border-b border-zinc-200 px-4 py-3 dark:border-zinc-800">
            ...
            {cart.length === 0 ? (
              ...
              <ul className="space-y-2">
                {cart.map((item) => (
                  <li
                    key={item.productId}
                    className="flex items-center gap-3 rounded-xl border border-zinc-200 bg-white p-2.5 dark:border-zinc-700 dark:bg-zinc-900"
                  >
          ...
          <div className="shrink-0 border-t border-zinc-200 p-4 dark:border-zinc-800">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <button
                ...
                disabled={cart.length === 0}
                className="flex items-center justify-center rounded-xl bg-[#C17F59] px-4 py-3.5 text-base font-bold text-white shadow-lg transition hover:bg-[#a96b48] disabled:cursor-not-allowed disabled:opacity-50 focus:outline-none focus:ring-2 focus:ring-[#C17F59] focus:ring-offset-2 dark:focus:ring-offset-zinc-950"
              >
                KREDİ KARTI
              </button>
            </div>
